    ...
    def get(self):
        ...
        if request.args.get('json'):
            ...
            data['favicon'] = self.config.get(
                'favicon',
                url_for('flasgger.static', filename='lib/swagger-ui/favicon-32x32.png')
            )
            data['swagger_ui_bundle_js'] = self.config.get(
                'swagger_ui_bundle_js',
                url_for('flasgger.static', filename='lib/swagger-ui/swagger-ui-bundle.js')
            )
            data['swagger_ui_standalone_preset_js'] = self.config.get(
                'swagger_ui_standalone_preset_js',
                url_for('flasgger.static',
                        filename='lib/swagger-ui/swagger-ui-standalone-preset.js')
            )
            data['jquery_js'] = self.config.get(
                'jquery_js',
                url_for('flasgger.static', filename='lib/jquery/jquery.min.js')
            )
            data['swagger_ui_css'] = self.config.get(
                'swagger_ui_css',
                url_for('flasgger.static', filename='lib/swagger-ui/swagger-ui.css')
            )
